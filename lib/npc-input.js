...
export class NpcInput extends FormApplication {
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      ...
      id: 'npc-input',
      template: 'systems/gurpsf/templates/npc-input.html',
      resizable: true,
      minimizable: false,
      width: 800,
      height: 700,
    ...
    let d = new Dialog(
      {
        ...
        content: await renderTemplate('systems/gurpsf/templates/import-stat-block.html', { block: b }),
        buttons: {
          import: {
  ...
  async populate(a) {
    ...
    data.basicmove.value = !!m.move ? parseInt(m.move) : m.rf / 2
    data.basicdodge.value = !!m.dodge ? parseInt(m.dodge) : Math.min(att.RF.value, att.AW.value)

    data.hearing = m.aw
    data.tastesmell = m.aw
    data.touch = m.aw
    data.vision = m.aw
    ...
    let hl = Object.values(hls).find(h => h.penalty == Math.max(Object.values(hls).map(x => x.penalty)))
    hl.dr = m.dr
    ...
    e.key = 'enc0'
    e.weight = 0
    e.move = data.basicmove.value
    e.dodge = data.basicdodge.value
    game.GURPS.put(es, e)
